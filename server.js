...
.replace(/&#x([0-9a-fA-F]+);/g, (*, hex) => String.fromCharCode(parseInt(hex, 16)))
.replace(/ /g, ’ ’)
.replace(/’/g, ‘\u2019’)
.replace(/‘/g, ‘\u2018’)
.replace(/”/g, ‘\u201D’)
...
html: ` <div style="font-family:Georgia,serif;max-width:520px;margin:0 auto;background:#fff;border:1px solid #eee;border-radius:8px;overflow:hidden;"> <div style="background:#FF2B2B;padding:24px 28px;"> <h1 style="color:#fff;margin:0;font-size:24px;letter-spacing:-0.5px;">Palermo Oggi</h1> <p style="color:rgba(255,255,255,0.8);margin:4px 0 0;font-size:12px;font-family:sans-serif;letter-spacing:2px;text-transform:uppercase;">Notizie da Palermo e dalla Sicilia</p> </div> <div style="padding:28px;"> <h2 style="margin:0 0 12px;font-size:18px;">Benvenuto! 🎉</h2> <p style="color:#444;line-height:1.7;margin:0 0 16px;">La tua iscrizione alla newsletter di <strong>Palermo Oggi</strong> è confermata.</p> <p style="color:#444;line-height:1.7;margin:0 0 16px;">Riceverai ogni mattina le ultime notizie da Palermo e dalla Sicilia direttamente nella tua casella email.</p> <a href="https://palermooggi.onrender.com" style="display:inline-block;background:#FF2B2B;color:#fff;text-decoration:none;padding:10px 22px;border-radius:4px;font-family:sans-serif;font-size:13px;font-weight:600;">Visita il sito →</a> </div> <div style="background:#f5f4f1;padding:14px 28px;font-family:sans-serif;font-size:11px;color:#888;"> Per disiscriverti rispondi a questa email con oggetto "Disiscrivi". </div> </div>`
});
} catch (err) {
...
const newsHtml = articles.map(a => ` <div style="border-top:1px solid #eee;padding:14px 0;"> <span style="font-family:sans-serif;font-size:10px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:#FF2B2B;">${a.cat}</span> <h3 style="margin:6px 0 4px;font-size:16px;line-height:1.3;">${a.title}</h3> <p style="color:#666;font-size:13px;margin:0 0 6px;line-height:1.5;">${(a.subtitle || '').slice(0, 120)}${(a.subtitle||'').length > 120 ? '…' : ''}</p> <span style="font-family:sans-serif;font-size:11px;color:#999;">${new Date(a.date).toLocaleDateString('it-IT',{day:'numeric',month:'long'})}</span> </div>`).join(’’);
...
html: ` <div style="font-family:Georgia,serif;max-width:520px;margin:0 auto;background:#fff;border:1px solid #eee;border-radius:8px;overflow:hidden;"> <div style="background:#FF2B2B;padding:20px 28px;display:flex;align-items:center;justify-content:space-between;"> <div> <h1 style="color:#fff;margin:0;font-size:22px;">Palermo Oggi</h1> <p style="color:rgba(255,255,255,0.75);margin:2px 0 0;font-size:11px;font-family:sans-serif;letter-spacing:1.5px;text-transform:uppercase;">Le notizie del giorno</p> </div> <span style="color:rgba(255,255,255,0.6);font-family:sans-serif;font-size:11px;">${new Date().toLocaleDateString('it-IT',{weekday:'long',day:'numeric',month:'long'})}</span> </div> <div style="padding:0 28px 8px;">${newsHtml}</div> <div style="padding:18px 28px;text-align:center;"> <a href="https://palermooggi.onrender.com" style="background:#FF2B2B;color:#fff;text-decoration:none;padding:10px 24px;border-radius:4px;font-family:sans-serif;font-size:13px;font-weight:600;display:inline-block;">Leggi tutte le notizie →</a> </div> <div style="background:#f5f4f1;padding:12px 28px;font-family:sans-serif;font-size:11px;color:#888;text-align:center;"> Per disiscriverti rispondi a questa email con oggetto "Disiscrivi". </div> </div>`
});
sent++;
} catch (err) {
